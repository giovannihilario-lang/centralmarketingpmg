...
(function () {
      ...
      try {
        ...
        const normalizados = (produtos || []).map((produto) => {
          const categoria = typeof window.categoriaPorId === 'function'
            ...
            : (produto.id_categoria ? `Categoria ${produto.id_categoria}` : 'Sem categoria');
          return {
            id: String(produto.id),
            codigo: String(produto.id),
            nome: produto.nome || `Produto ${produto.id}`,
            fornecedor: produto.fornecedor || '',
            grupo: categoria,
            subgrupo: produto.id_subcategoria ? `Subcategoria ${produto.id_subcategoria}` : '',
        ...
        state.catalogoVisualLista = normalizados;
        state.catalogoVisual = new Map(normalizados.map((produto) => [String(produto.id), produto]));
      } catch (erro) {
  ...
  function renderizarCatalogo() {
    ...
    const total = document.getElementById('cbTotalFiltrado');
    if (total) total.textContent = `${state.catalogo.length} produto(s) carregado(s)`;
  }
  ...
  async function carregarFiltros(forcar = false) {
    ...
    try {
      ...
      const base = {
        fornecedores: fornecedor ? [fornecedor] : valoresUnicos(listaFornecedor, 'fornecedor'),
        grupos: valoresUnicos(listaFornecedor, 'grupo'),
        subgrupos: valoresUnicos(listaFornecedor, 'subgrupo'),
        status: valoresUnicos(listaFornecedor, 'status'),
      };
      state.filtros = base;
      state.filtrosChave = chave;
  ...
  window.cbAdicionarFiltrados = function () {
    if (!state.catalogo.length) {
      if (typeof showToast === 'function') showToast('Nenhum produto filtrado para adicionar.', true);
      return;
    }
    ...
    if (!destino) return;
    let adicionados = 0;
    state.catalogo.forEach((produto) => {
      ...
      destino.produtos.push({ ...produto, codigo: String(produto.codigo), id: String(produto.id || produto.codigo) });
