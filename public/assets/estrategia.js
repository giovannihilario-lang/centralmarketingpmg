...
function buildOverviewSlides(){
  ...
  return [
    ...
    {kicker:'01 · Faturamento e peso',title:growthValor==null?'O ano em números, lado a lado':`Faturamento ${growthValor>=0?'cresceu':'caiu'} ${deltaText(growthValor).replace(/[▲▼]\s*/,'')} na comparação anual`,metrics:[['Faturamento '+y.previous.label,money(y.kpisPrev.total_valor)],['Faturamento '+y.current.label,money(y.kpisCur.total_valor)],['Variação de faturamento',growthValor==null?'—':deltaText(growthValor)],['Peso '+y.previous.label,kg(y.kpisPrev.total_kg)],['Peso '+y.current.label,kg(y.kpisCur.total_kg)],['Variação de peso',growthKg==null?'—':deltaText(growthKg)]],subtitle:'Base: mesmos meses fechados nos dois anos, para uma comparação justa.',source:'Fonte: SQL Server · dbo.Vendas / dbo.VendasProdutos'},
    metaMensalSlide(),
    {kicker:'03 · Por região',title:topBottomHeadline(y.regiao,'região'),list:yoyRankingList(y.regiao,money),subtitle:'3 maiores crescimentos e 3 maiores quedas em faturamento, região com base comparável nos dois anos.',source:'Fonte: SQL Server · dbo.Clientes.Zona'},
    {kicker:'04 · Por segmento',title:topBottomHeadline(y.segmento,'segmento'),list:yoyRankingList(y.segmento,money),subtitle:'Mesmo recorte, agora por segmento de cliente.',source:'Fonte: SQL Server · dbo.Clientes.Segmento'},
...
function slideListRowHtml(item){
  ...
  return `<div class="slide-list-row"><div class="slide-list-head"><strong>${esc(l)}</strong><span class="${negative?'is-negative':''}">${esc(v)}</span></div>${bar}</div>`;
}
