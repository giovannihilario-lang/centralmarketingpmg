...
function buildOverviewSlides(){
  ...
  return [
    ...
    {icon:'banknote',kicker:'01 · Faturamento e peso',title:growthValor==null?'O ano em números, lado a lado':`Faturamento ${growthValor>=0?'cresceu':'caiu'} ${deltaText(growthValor).replace(/[▲▼]\s*/,'')} na comparação anual`,metrics:[['Faturamento '+y.previous.label,money(y.kpisPrev.total_valor)],['Faturamento '+y.current.label,money(y.kpisCur.total_valor)],['Variação de faturamento',growthValor==null?'—':deltaText(growthValor),growthValor>=0?'trending-up':'trending-down'],['Peso '+y.previous.label,kg(y.kpisPrev.total_kg)],['Peso '+y.current.label,kg(y.kpisCur.total_kg)],['Variação de peso',growthKg==null?'—':deltaText(growthKg),growthKg>=0?'trending-up':'trending-down']],subtitle:'Base: mesmos meses fechados nos dois anos, para uma comparação justa.',source:'Fonte: SQL Server · dbo.Vendas / dbo.VendasProdutos'},
    metaMensalSlide(),
