...
function buildOpportunityActionSlides(){
  ...
  return [
    {kind:'cover',icon:'target',kicker:'PMG · Planejamento Estratégico · Apresentação 2 de 2',title:'Oportunidades e plano de ação',subtitle:`A partir do retrato atual, para onde crescemos e o que cada área assume nos próximos 90 dias. Período-base: ${d.period}.`},
    {icon:'lightbulb',kicker:'01 · Oportunidades',title:'Sinais comerciais que merecem investigação',list:d.topOps.map(op=>[op.title,`Score ${op.score.toFixed(0)} · ${opportunityTypeLabel(op)}`,Math.max(6,Math.round(op.score))]),subtitle:'Sinais baseados em comparação histórica interna. Não são previsão absoluta de mercado.'},
    {icon:'layout-grid',kicker:'02 · Portfólio',title:'Transformando oportunidade em execução',metrics:[['Projetos ativos',num(d.summary.active),'folder-kanban'],['No ritmo / atingidos',num(d.summary.achieved+Math.max(0,d.summary.active-d.summary.risk-d.summary.attention-d.summary.below-d.summary.achieved)),'circle-check'],['Em risco / atenção',num(d.summary.risk+d.summary.attention+d.summary.below),'circle-alert'],['Metas de faturamento comprometidas',money(d.summary.committedPotential),'banknote']],subtitle:'Cada projeto preserva o baseline e mede o resultado ao longo de 90 dias.'},
    ...(d.selected?[{icon:'folder-kanban',kicker:'03 · Exemplo de projeto',title:d.selected.titulo,subtitle:d.selected.objetivo,metrics:[['Baseline',formatProjectMetric(d.selected,normalizeKpis(safeJson(d.selected.baseline,{}))),'flag'],['Meta',d.selected.meta_tipo==='percentual'?`${number(d.selected.meta_valor).toFixed(1)}%`:money(d.selected.meta_valor),'target'],['Resultado atual',d.selected.meta_tipo==='percentual'?`${selectedProgress.delta.toFixed(1)}%`:formatProjectMetric(d.selected,normalizeKpis(projectLatest(d.selected)||{})),'trending-up'],['Status',healthLabel(selectedProgress.health),'gauge']]}]:[]),
    {icon:'users-round',kicker:'04 · Áreas conectadas',title:'O plano de ação é interdepartamental',actions:d.actions.slice(0,8),subtitle:d.selected?`Ações vinculadas ao projeto “${d.selected.titulo}”.`:'Comercial, Logística, Marketing, Compras, Financeiro e Estoque entram conforme a oportunidade.'},
    {icon:'calendar-clock',kicker:'05 · 90 dias',title:'Executar, medir e corrigir rota',columns:[['Baseline','Fotografia dos indicadores no início.','flag'],['Mês 1','Primeira medição e remoção de bloqueios.','footprints'],['Mês 2','Ajustes e reforço do que está performando.','settings-2'],['Mês 3','Fechamento, aprendizados e decisão de escalar ou recalcular.','flag-triangle-right']]},
    {icon:'shield-check',kicker:'06 · Governança',title:'Um número precisa ter fonte e responsável',columns:[['Dados','Regional e SQL comercial alimentam indicadores.','database'],['Execução','Ações estratégicas podem virar Demandas.','workflow'],['Histórico','Baseline e medições ficam preservados no Supabase.','history'],['Decisão','Resultado final registra o que funcionou, gargalos e próximo passo.','gavel']]},
