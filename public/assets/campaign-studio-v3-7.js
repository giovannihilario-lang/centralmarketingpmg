/* PMG Connect — Campaign Studio 3.7 — performance no padrão Dashboard Regional
...
(() => {
  ...
  const atualizarIcones = () => {
    if (!window.lucide || _iconeJob) return;
    const executar = () => {
      _iconeJob = null;
      try { window.lucide.createIcons({ attrs: { 'stroke-width': 1.9 } }); } catch (_) {}
    };
    // O Regional deixa a interface pintar antes de tarefas decorativas. Ícones seguem a mesma regra.
    _iconeJob = window.requestIdleCallback
      ? window.requestIdleCallback(executar, { timeout: 900 })
      : window.setTimeout(executar, 220);
  };
  /* Cache e deduplicação no mesmo padrão do Dashboard Regional. */
  const SQL_CACHE_TTL_MS = 60_000;
  const _sqlCache = new Map();
  const _sqlInFlight = new Map();

  function chaveApi(url, options = {}) {
  ...
  async function fetchJsonSeguro(url, options = {}) {
    const metodo = String(options.method || 'GET').toUpperCase();
    const podeCachear = metodo === 'GET' && !options.forceRefresh;
    const chave = chaveApi(url, options);

    if (podeCachear) {
      const cache = _sqlCache.get(chave);
      if (cache && Date.now() - cache.salvoEm < SQL_CACHE_TTL_MS) return cache.dados;
      if (_sqlInFlight.has(chave)) return _sqlInFlight.get(chave);
    }

    const executar = async () => {
      const controller = new AbortController();
      const timeout = window.setTimeout(() => controller.abort(), Number(options.timeout || 12_000));
      let resposta;
      try {
        resposta = await fetch(url, { ...options, signal: controller.signal });
      } catch (erroRede) {
        const abortou = erroRede?.name === 'AbortError';
        const erro = new Error(abortou
          ? `A API local da PMG demorou mais de 12 segundos para responder.`
          : `Não foi possível acessar a API local da PMG em ${CAMPANHAS_SQL_API_BASE}. Abra o terminal na pasta do projeto e execute npm start.`);
        erro.codigo = abortou ? 'API_LOCAL_TIMEOUT' : 'API_LOCAL_INDISPONIVEL';
      ...
      if (podeCachear) _sqlCache.set(chave, { salvoEm: Date.now(), dados });
      return dados;
    };

    const promessa = executar().finally(() => _sqlInFlight.delete(chave));
    if (podeCachear) _sqlInFlight.set(chave, promessa);
    return promessa;
  }
  ...
  async function carregarFornecedores({ busca = '', forcar = false } = {}) {
    ...
    if (!fornecedoresCache.length) fornecedoresCache = lerFornecedoresCacheLocal();

    // Abrir o editor não dispara SQL. Sem termo, devolvemos apenas o cache recente.
    if (!termo) return fornecedoresCache.slice(0, 12);
    if (termo.length < 2) {
      return fornecedoresCache.filter((item) => normalizeKey(item.nome).includes(normalizeKey(termo))).slice(0, 12);
    }

    const params = new URLSearchParams({ recurso: 'fornecedores', limite: '24', busca: termo });
    const dados = await fetchJsonSeguro(`${CAMPANHAS_SQL_ENDPOINT}?${params.toString()}`, { forceRefresh: forcar });
    const lista = normalizarListaFornecedores(dados);

    const mesclados = new Map(fornecedoresCache.map((item) => [normalizeKey(item.nome), item]));
    lista.forEach((item) => mesclados.set(normalizeKey(item.nome), item));
    fornecedoresCache = [...mesclados.values()].slice(0, 200);
    salvarFornecedoresCacheLocal(fornecedoresCache);
    return lista;
  }
  ...
  function renderFornecedorCards(lista, opcoes = {}) {
    ...
    if (opcoes.carregando) {
      grade.innerHTML = `<div class="cs-supplier-loading"><span class="cs-mini-spinner"></span> Pesquisando no SQL Server...</div>`;
      return;
    }
    if (opcoes.erro) {
      grade.innerHTML = `<div class="cs-supplier-error">
        <strong>Não foi possível consultar os fornecedores.</strong>
        <span>${esc(opcoes.erro.message || opcoes.erro)}</span>
        <small>${esc(opcoes.erro.dica || 'Confirme se o npm start está aberto.')}</small>
        <button type="button" class="btn btn-primary btn-sm" onclick="csRecarregarFornecedores()">Tentar novamente</button>
      </div>`;
      return;
    }

    const listaVisivel = (lista || []).slice(0, termo ? 24 : 12);
    if (!listaVisivel.length && !termo) {
      grade.innerHTML = `<div class="cs-supplier-prompt"><strong>Busque o fornecedor</strong><span>Digite pelo menos 2 letras. Nada é carregado antes disso, deixando o editor imediato.</span></div>`;
      return;
    }
    if (!listaVisivel.length) {
      grade.innerHTML = `<div class="cs-supplier-empty">Nenhum fornecedor encontrado para “${esc(termo)}”.</div>`;
      return;
    }
    ...
    grade.innerHTML = listaVisivel.map((fornecedor) => `<button type="button" class="cs-supplier-card ${normalizeKey(fornecedor.nome) === normalizeKey(selecionado) ? 'active' : ''}" data-id="${esc(String(fornecedor.id ?? ''))}" data-nome="${esc(fornecedor.nome)}" onclick="csSelecionarFornecedor(this.dataset.id, this.dataset.nome)">
      ...
      <span class="cs-supplier-check">✓</span>
    </button>`).join('');
  }
  ...
  async function consultarFornecedoresNaTela(termo = '', forcar = false) {
    ...
    renderFornecedorCards([], { carregando: true, termo: busca });
    try {
      const lista = await carregarFornecedores({ busca, forcar });
      renderFornecedorCards(lista, { termo: busca });
    } catch (erro) {
      renderFornecedorCards([], { erro, termo: busca });
    } finally {
  ...
  function instalarSeletorFornecedor() {
    ...
    campo.insertAdjacentHTML('afterend', `<section class="field full cs-supplier-studio" id="csFornecedorStudio">
      <div class="cs-supplier-heading"><div><label>Fornecedor da campanha *</label><p>Pesquise e selecione. Produtos e apuração serão limitados a ele.</p></div><span class="cs-source-badge">SQL · dbo.Produtos</span></div>
      <div class="cs-supplier-search"><span aria-hidden="true">⌕</span><input id="csFornecedorBusca" autocomplete="off" placeholder="Digite ao menos 2 letras" oninput="csFiltrarFornecedores(this.value)"><button type="button" class="cs-supplier-refresh" onclick="csRecarregarFornecedores()" title="Refazer pesquisa">↻</button></div>
    ...
    renderFornecedorCards(cache.slice(0, 12));
  }
  ...
  window.csFiltrarFornecedores = function (termo) {
    ...
    fornecedorBuscaTimer = setTimeout(() => consultarFornecedoresNaTela(busca, true), 280);
  };

  window.csRecarregarFornecedores = async function () {
    const termo = String(document.getElementById('csFornecedorBusca')?.value || '').trim();
    if (termo.length < 2) {
      showToast('Digite ao menos 2 letras para pesquisar o fornecedor.', true);
      document.getElementById('csFornecedorBusca')?.focus();
      return;
    }
  ...
  function csPrimeiroFrameModal(titulo) {
    const title = document.getElementById('modalTitle');
    ...
    if (body) body.innerHTML = `<div class="cs-modal-first-paint"><strong>${esc(titulo)}</strong><span>Abrindo o editor...</span></div>`;
    if (foot) foot.innerHTML = `<button class="btn btn-ghost" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" disabled>Salvar Campanha</button>`;
    box?.classList.add('wide', 'campaign-editor', 'cs-fast-editor');
    overlay?.classList.add('show');
    document.body.classList.add('cs-campaign-editor-open');
  }
  ...
  window.openCampanhaModal = function abrirCampanhaInstantanea(id) {
    ...
    // Primeiro task: apenas torna o modal visível. Nenhum DB, SQL, ícone ou formulário.
    csPrimeiroFrameModal(id ? 'Editar Campanha' : 'Nova Campanha');

    // Segundo task: monta apenas a casca do editor depois que o navegador já pôde pintar.
    setTimeout(() => {
      if (token !== csModalLazy.token) return;
      const body = document.getElementById('modalBody');
      const foot = document.getElementById('modalFoot');
      if (body) body.innerHTML = csEditorShell();
      if (foot) foot.innerHTML = `<button class="btn btn-ghost" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="csSalvarCampanhaProgressiva()">Salvar Campanha</button>`;
      csAplicarChromeEditor();
      csAtualizarProgressoEtapa(0);

      // Terceiro task: formulário. Campanha nova não faz nenhuma leitura do IndexedDB.
      setTimeout(() => {
        if (token !== csModalLazy.token) return;
        if (!id) {
          csModalLazy.campanha = csCampanhaVazia();
          csModalLazy.regras = [];
          csModalLazy.regrasProduto = [];
          _campanhaModalState = { campanha: csModalLazy.campanha, regras: [], regrasProduto: [] };
          csRenderizarEtapa(0);
        } else {
          csCarregarDadosEditor(id, token);
        }
        atualizarIcones();
      }, 0);
    }, 0);
  ...
  const fecharModalBase = window.closeModal;
  window.closeModal = function fecharModalCampanha() {
    document.body.classList.remove('cs-campaign-editor-open');
    return fecharModalBase();
  };

  document.addEventListener('DOMContentLoaded', () => {
    // Decoração só no tempo ocioso. O conteúdo funcional nunca espera pelos ícones.
    setTimeout(atualizarIcones, 700);
  });
})();
