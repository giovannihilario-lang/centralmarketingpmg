...
-- HOTFIX DEFINITIVO: recorrencias legadas + conclusao/autoria
-- Execute este arquivo uma vez no Supabase SQL Editor.

begin;

-- 1) Corrige estados históricos antes de recriar o CHECK.
alter table if exists public.demandas_recorrentes_ocorrencias
  drop constraint if exists demandas_recorrentes_ocorrencias_estado_check;

update public.demandas_recorrentes_ocorrencias
set estado = case
  when lower(coalesce(estado, '')) in ('gerada','gerado','criada','criado','pendente','realizada','realizado','concluida','concluído','concluido') then 'gerada'
  when lower(coalesce(estado, '')) in ('pulada','pulado','ignorada','ignorado','cancelada','cancelado') then 'pulada'
  when lower(coalesce(estado, '')) in ('nao_realizada','não_realizada','nao-realizada','não-realizada','perdida','atrasada') then 'nao_realizada'
  when tarefa_id is not null then 'gerada'
  when data_referencia < current_date then 'nao_realizada'
  else 'gerada'
end
where estado is null
   or lower(estado) not in ('gerada','pulada','nao_realizada');

alter table public.demandas_recorrentes_ocorrencias
  add constraint demandas_recorrentes_ocorrencias_estado_check
  check (estado in ('gerada','pulada','nao_realizada'));

commit;

-- 2) Reinstala a autoria corrigida da V3.8.6.
--    O bloco abaixo é idempotente e pode ser executado mesmo se o SQL 15/16 já tiver sido aplicado.
-- ============================================================
...
begin
  ...
  -- Trabalho individual: sem confirmação extra e sem depender de
  -- avaliar_conclusao(). Algumas bases antigas possuem uma versão dessa
  -- RPC que bloqueia a conclusão exigindo autoria, criando um ciclo.
  if v_qtd = 1 then
    ...
    values (
      ...
      'avaliacao',
      jsonb_build_object(
        'resultado', 'aprovada_individual',
        'executor', v_executores[1],
...
select
  true as recorrencias_reparadas,
  to_regprocedure('public.solicitar_confirmacao_autoria_v1(uuid,uuid[],text)') is not null as validar_entrega_ok,
  to_regprocedure('public.responder_confirmacao_autoria_v1(uuid,boolean,text)') is not null as confirmar_autoria_ok,
