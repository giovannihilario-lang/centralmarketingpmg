...
// Handlers compartilhados com api/wave2/[...route].js (producao na Vercel),
// para as duas nunca divergirem entre si.
// ------------------------------------------------------------
for (const [routeKey, routeHandler] of Object.entries(WAVE2_ROUTES)) {
  const [method, routePath] = routeKey.split(' ');
  app[method.toLowerCase()](`/api/wave2${routePath}`, routeHandler);
}
